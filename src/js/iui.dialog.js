...
;(function ( $, window, undefined ) {
    ...
    var template = '\
				<div class="core_pop">\
					<% if(type === "iframe" || type === "html"  || type === "ajaxhtml") {%>\
						<div class="pop_top J_drag_handle" style="display:none;overflow:hidden;">\
							<a role="button" href="#" class="pop_close J_close" title="关闭弹出窗口"><i class="fa fa-close"></i></a>\
							<strong><%=title%></strong>\
						</div>\
					<% } %>\
			    	<% if(type === "ajaxhtml") { %>\
    ...
    Plugin.prototype.init = function () {
        ...
        if(options.isMask) {//遮罩
    		...
    		_this.mask = $('<div class="iui_dialog_mask"/>').css(style).appendTo('body');
            //ie6则调用bgiframe组件
            if (is_ie6) {
                Iui.use('bgiframe',function() {
                    _this.mask.bgiframe();
                });
            }
    	}
        ...
        if(options.type === 'iframe' || options.isDrag || options.type=="html"|| options.type=="ajaxhtml") {
        	iui.use('draggable',function() {
        		elem.draggable( { handle : '.J_drag_handle'} );
        	});
        }
        ...
        if(options.type === 'html' && options.title) {
            ...
            elem.css('width',$(window.document).width()/2 + 'px');
            pop_top.show();
        }
        if(options.type === 'ajaxhtml' && options.title) {
            loading = elem.find('.J_loading');
            loading.show();
			pop_top.hide();
			show();
            ...
            elem.children().find(".pop_cont").html(msg).show();
            loading.hide();
            pop_top.show();
            elem.css('width',$(window.document).width()/2 + 'px');
        }
    ...
    /*$.fn[pluginName] = Wind[pluginName] = function ( options ) {
   	...
   	var iui = window.iui || {};
	var dialog = iui[pluginName] = function(options) {
		return new Plugin( options );
	};

	dialog['alert'] = iui['alert'] = function(message,callback) {//兼容api
		return new Plugin( { message:message, type:'alert', onOk:callback } );
	};
	dialog['confirm'] = iui['confirm'] = function(message,okCallback,cancelCallback) {
	...
	dialog['open'] = iui['showUrl'] = function(url,options) {
        options = options || {};
		options['type'] = 'iframe';
		options['url'] = url;
		return new Plugin( options );
	};
	dialog['html'] = iui.showHTML = function(html,options) {
        options = options || {};
		options['type'] = 'html';
		options['message'] = html;
		return new Plugin( options );
	};
	dialog['ajaxOpen'] = iui['showUrl'] = function(url,options) {
        options = options || {};
		options['type'] = 'ajaxhtml';
		options['url'] = url;
		return new Plugin( options );
	};
