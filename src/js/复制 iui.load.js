...
*/
(function(win) {
	var doc = document, ver = typeof win.GV !== 'undefined' && typeof win.GV.VERSION !== 'undefined'? win.GV.VERSION.replace(/(\s)/g, '_'):'1.0',
	config = {
	    modules: {} //记录模块物理路径
	 },
 	getPath = function(){
    ...
    ,mui: 'libs/mui/js/mui.min'//表单异步提交
...
var getDps = (function dps(dependency,arr){
...
//
	...
	iui.use = function() {
		var args = arguments,len = args.length,all=[];
		
        for( var i = 0;i < len;i++ ) {
	        	if(typeof args[i] === 'string' && modules[args[i]]) {
	        		//获取依赖关系
	        		var dps = getDps(args[i],[]).unique();
