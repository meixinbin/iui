...
(function($){
  ...
  var laytpl = iui.tpl,laypage = iui.laypage,layer = iui.layer,form = iui.form
  ...
  ,table = {
    ...
    ,cache: {} //数据缓存
    ,index: iui.table ? (iui.table.index + 10000) : 0
    //设置全局项
  ...
  //字符常量
  ,MOD_NAME = 'table', ELEM = '.iui-table', THIS = 'iui-this', SHOW = 'iui-show', HIDE = 'iui-hide', DISABLED = 'iui-disabled', NONE = 'iui-none'
  ...
  ,TPL_HEADER = function(options){
    ...
    options = options || {};
    return ['<table cellspacing="0" cellpadding="0" border="0" class="iui-table" '
      ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
      ,'<thead>'
      ,'{{# iui.each(d.data.cols, function(i1, item1){ }}'
        ,'<tr>'
        ,'{{# iui.each(item1, function(i2, item2){ }}'
          ...
          ,'<th data-field="{{ item2.field||i2 }}" {{# if(item2.minWidth){ }}data-minwidth="{{item2.minWidth}}"{{# } }} '+ rowCols +' {{# if(item2.unresize){ }}data-unresize="true"{{# } }}>'
            ,'<div class="iui-table-cell laytable-cell-'
              ,'{{# if(item2.colspan > 1){ }}'
                ,'group'
              ,'{{# } else { }}'
            ...
            ,'" {{#if(item2.align){}}align="{{item2.align}}"{{#}}}>'
              ...
              ,'{{# } else { }}'
                ...
                ,'{{# if(!(item2.colspan > 1) && item2.sort){ }}'
                  ,'<span class="iui-table-sort iui-inline"><i class="iui-edge iui-table-sort-asc"></i><i class="iui-edge iui-table-sort-desc"></i></span>'
                ,'{{# } }}'
              ,'{{# } }}'
            ,'</div>'
          ,'</th>'
  ...
  //tbody区域模板
  ,TPL_BODY = ['<table cellspacing="0" cellpadding="0" border="0" class="iui-table" '
    ,'{{# if(d.data.skin){ }}lay-skin="{{d.data.skin}}"{{# } }} {{# if(d.data.size){ }}lay-size="{{d.data.size}}"{{# } }} {{# if(d.data.even){ }}lay-even{{# } }}>'
    ,'<tbody></tbody>'
  ,'</table>'].join('')
  
  //主模板
  ,TPL_MAIN = ['<div class="iui-form iui-border-box {{d.VIEW_CLASS}}" lay-filter="LAY-table-{{d.index}}" style="{{# if(d.data.width){ }}width:{{d.data.width}}px;{{# } }} {{# if(d.data.height){ }}height:{{d.data.height}}px;{{# } }}">'
    
    ,'{{# if(d.data.toolbar){ }}'
    ,'<div class="iui-table-tool"></div>'
    ,'{{# } }}'
    ...
    ,'<div class="iui-table-box">'
      ...
      ,'<div class="iui-table-header">'
        ,TPL_HEADER()
      ,'</div>'
      ,'<div class="iui-table-body iui-table-main">'
        ,TPL_BODY
      ,'</div>'
      
      ,'{{# if(left){ }}'
      ,'<div class="iui-table-fixed iui-table-fixed-l">'
        ,'<div class="iui-table-header">'
      ...
      ,'{{# if(right){ }}'
      ,'<div class="iui-table-fixed iui-table-fixed-r">'
        ,'<div class="iui-table-header">'
          ,TPL_HEADER({fixed: 'right'})
          ,'<div class="iui-table-mend"></div>'
        ,'</div>'
        ,'<div class="iui-table-body">'
          ,TPL_BODY
        ,'</div>'
    ...
    ,'{{# if(d.data.page){ }}'
    ,'<div class="iui-table-page">'
      ,'<div id="iui-table-page{{d.index}}"></div>'
    ,'</div>'
    ,'{{# } }}'
  ...
  Class.prototype.setArea = function(){
    ...
    //遍历所有列
    iui.each(options.cols, function(i1, item1){
      iui.each(item1, function(i2, item2){
        var width;
  ...
  Class.prototype.eachCols = function(callback){
    ...
    //重新整理表头结构
    iui.each(cols, function(i1, item1){
      iui.each(item1, function(i2, item2){
        //如果是组合列，则捕获对应的子列
        if(item2.colspan > 1){
          var childIndex = 0;
          index++
          item2.CHILD_COLS = [];
          iui.each(cols[i1 + 1], function(i22, item22){
    ...
    var eachArrs = function(obj){
      iui.each(obj || arrs, function(i, item){
  ...
  Class.prototype.renderData = function(res, curr, count, sort){
    ...
    ,render = function(){ //后续性能提升的重点
      ...
      iui.each(data, function(i1, item1){
        ...
        that.eachCols(function(i3, item3){
          ...
          var td = ['<td data-field="'+ field +'" '+ function(){
            ...
            ,'<div class="iui-table-cell laytable-cell-'+ function(){ //返回对应的CSS类标识
    ...
    if(options.page){
      options.page = $.extend({
        elem: 'iui-table-page' + options.index
        ,count: count
        ,limit: options.limit
        ,limits: options.limits || [10,20,30,40,50,60,70,80,90]
        ,groups: 3
        ,layout: ['prev', 'page', 'next', 'skip', 'count', 'limit']
        ,prev: '<i class="iui-icon">&#xe603;</i>'
        ,next: '<i class="iui-icon">&#xe602;</i>'
  ...
  Class.prototype.sort = function(th, type, pull, formEvent){
    var that = this,field,res = {},options = that.config
    ...
    if(type === 'asc'){ //升序
      thisData = iui.sort(data, field);
    } else if(type === 'desc'){ //降序
      thisData = iui.sort(data, field, true);
    } else { //清除排序
      thisData = iui.sort(data, table.config.indexName);
      delete that.sortKey;
    }
    ...
    if(formEvent){
      iui.event.call(th, MOD_NAME, 'sort('+ filter +')', {
        field: field
        ,type: type
      });
    }
  ...
  Class.prototype.getCssRule = function(field, callback){
    ...
    iui.each(rules, function(i, item){
  ...
  Class.prototype.scrollPatch = function(){
    ...
    if(scollWidth && scollHeight){
      if(!that.elem.find('.iui-table-patch')[0]){
        var patchElem = $('<th class="iui-table-patch"><div class="iui-table-cell"></div></th>'); //补丁元素
      ...
      that.layHeader.eq(0).find('.iui-table-patch').remove();
    }
  ...
  Class.prototype.events = function(){
    ...
    }).find(ELEM_SORT+' .iui-edge ').on('click', function(e){
      var othis = $(this)
      ,index = othis.index()
    ...
    that.elem.on('click', 'input[name="layTableCheckbox"]+', function(){
      ...
      iui.event.call(this, MOD_NAME, 'checkbox('+ filter +')', {
        checked: checked
    ...
    that.layBody.on('change', '.'+ELEM_EDIT, function(){
      ...
      iui.event.call(this, MOD_NAME, 'edit('+ filter +')', {
        value: value
        ,data: data
        ,field: field
      });
    ...
    that.layBody.on('click', 'td', function(){
      ...
      if(editType){
        if(editType === 'select') { //选择框
          ...
          var input = $('<input class="iui-input '+ ELEM_EDIT +'">');
      ...
      //如果出现省略，则可查看更多
      if(elemCell.find('.iui-form-switch,.iui-form-checkbox')[0]) return; //限制不出现更多（暂时）
      
      if(Math.round(elemCell.prop('scrollWidth')) > Math.round(elemCell.outerWidth())){
        that.tipsIndex = layer.tips([
          '<div class="iui-table-tips-main" style="margin-top: -'+ (elemCell.height() + 16) +'px;'+ function(){
            if(options.size === 'sm'){
            ...
            ,elemCell.html()
          ,'</div>'
          ,'<i class="iui-icon iui-table-tips-c">&#x1006;</i>'
        ].join(''), elemCell[0], {
          tips: [3, '']
          ,time: -1
          ,anim: -1
          ...
          ,success: function(layero, index){
            layero.find('.iui-table-tips-c').on('click', function(){
              layer.close(index);
            });
          }
        });
    ...
    that.layBody.on('click', '*[lay-event]', function(){
      ...
      ,ELEM_CLICK = 'iui-table-click'
      ,data = table.cache[that.key][index];
      
      iui.event.call(this, MOD_NAME, 'tool('+ filter +')', {
        ...
        ,update: function(fields){
          fields = fields || {};
          iui.each(fields, function(key, value){
            if(key in data){
  ...
  table.init = function(filter, settings){
    ...
    elemTable.each(function(){
      ...
      othis.find('tbody>tr').each(function(i1){
        ...
        //如果未定义字段名
        iui.each(cols, function(i3, item3){
