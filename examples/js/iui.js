 ...
 * =====================================================
 */
var doc = document,getPath = function(){
...
alert(getPath)
var root = GV.ASSETS_ROOT || location.origin;
var alias_js = {
	jquery: root + '/libs/jquery/jquery',
	bootstrap: root + '/libs/bootstrap/js/bootstrap.min',
	codemirror: root + '/libs/codemirror/lib/codemirror'
};
var alias_css = {
	colorPicker: root + 'ui_libs/colorPicker/style',
	datePicker: root + 'ui_libs/datePicker/style',
	chosen: root + 'ui_libs/chosen/chosen'
};
/**
 * iui核心JS
 * @type _L4.$|Function
 */
var iui = (function(document, undefined) {
	...
	/**
	 * iui isWindow(需考虑obj为undefined的情况)
	 */
...
*/
(function(win) {
	var ver = win.GV.VERSION.replace(/(\s)/g, '_') || '1.0';
		
	//add suffix and version
	for(var i in alias_js) {
		if (alias_js.hasOwnProperty(i)) {
			alias_js[i] = alias_js[i] +'.js?v=' + ver;
		}
	}
	...
	iui.use = function() {
        ...
        for( var i = 0;i < len;i++ ) {
        	if(typeof args[i] === 'string' && alias_js[args[i]]) {
        		args[i] = alias_js[args[i]];
        	}
        }
		iui.js.apply(null,args);
	};
...
(function($, document) {
	function detect(ua) {
		...
		if (plus) {
			this.os.plus = true;
			$(function() {
				document.body.classList.add('iui-plus');
			});
			if (ua.match(/StreamApp/i)) { //TODO 最好有流应用自己的标识
				this.os.stream = true;
				$(function() {
					document.body.classList.add('iui-plus-stream');
				});
			}
		}
	}
...
(function($) {
	...
	var delegateFn = function(element, event, selector, callback) {
		return function(e) {
			...
			for (; target && target !== document; target = target.parentNode) {
				...
				if (~['click', 'tap', 'doubletap', 'longtap', 'hold'].indexOf(event) && (target.disabled || target.classList.contains('iui-disabled'))) {
					break;
				}
				var matches = {};
	...
	$.fn.off = function(event, selector, callback) {
		return this.each(function() {
			var _mid = mid(this);
			if (!event) { //iui(selector).off();
				delegates[_mid] && delete delegates[_mid];
			} else if (!selector) { //iui(selector).off(event);
				delegates[_mid] && delete delegates[_mid][event];
			} else if (!callback) { //iui(selector).off(event,selector);
				delegates[_mid] && delegates[_mid][event] && delete delegates[_mid][event][selector];
			} else { //iui(selector).off(event,selector,callback);
...
/**
 * iui target(action>popover>modal>tab>toggle)
 */
...
/**
 * iui fixed requestAnimationFrame
 * @param {type} window
 * @returns {undefined}
 */
(function(window) {
...
(function($, document) {
	$(function() {
		...
		var CLASS_FOCUSIN = 'iui-focusin';
		var CLASS_BAR_TAB = 'iui-bar-tab';
		var CLASS_BAR_FOOTER = 'iui-bar-footer';
		var CLASS_BAR_FOOTER_SECONDARY = 'iui-bar-footer-secondary';
		var CLASS_BAR_FOOTER_SECONDARY_TAB = 'iui-bar-footer-secondary-tab';
...
/**
 * iui gesture flick[left|right|up|down]
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
...
/**
 * iui gesture swipe[left|right|up|down]
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
...
/**
 * iui gesture drag[start|left|right|up|down|end]
 * @param {type} $
 * @param {type} name
 * @returns {undefined}
 */
	...
	var handle = function(event, touch) {
		...
		switch (event.type) {
			case $.EVENT_END:
				...
				if (!target || (target.disabled || (target.classList && target.classList.contains('iui-disabled')))) {
					return;
				}
...
(function($) {
	...
	var createIframe = function(options) {
		...
		wrapper.className = 'iui-iframe-wrapper';
	...
	function handleScroll(wrapper, iframe) {
		var key = 'iui_SCROLL_POSITION_' + document.location.href + '_' + iframe.src;
	...
	$(function() {
		...
		if ($.os.ios) {
			...
			classList.add('iui-ios');
		} else if ($.os.android) {
			os.push({
				os: 'android',
				version: $.os.version
			});
			classList.add('iui-android');
		}
		if ($.os.wechat) {
			os.push({
				os: 'wechat',
...
(function($) {
	...
	$.fire = function(webview, eventType, data) {
		if (webview) {
			...
			webview.evalJS("typeof iui!=='undefined'&&iui.receive('" + eventType + "','" + data + "')");
		}
	};
	/**
	 * 5+ event(5+没提供之前我自己实现)
	...
	$.openWindow = function(url, id, options) {
		...
		if (webviewCache && webview) { //已缓存
			//每次show都需要传递动画参数；
			//预加载的动画参数优先级：openWindow配置>preloadPages配置>iui默认配置；
	...
	$.addInit({
		...
		handle: function() {
			...
			if ($.os.plus) {
				...
				//					err.className = 'iui-error';
				//					//文字描述
				//					var span = document.createElement('span');
				//					span.innerHTML = '在该浏览器下，不支持创建子页面，具体参考';
				//					err.appendChild(span);
				//					var a = document.createElement('a');
				//					a.innerHTML = '"iui框架适用场景"';
	...
	$.ready(function() {
		...
		if ($.supportStatusbarOffset()) {
			document.body.classList.add('iui-statusbar');
		}
	});
})(iui);
/**
...
(function($, window) {
	...
	window.addEventListener('tap', function(e) {
		...
		if (action && action.classList.contains('iui-action-back')) {
			$.back();
...
(function($, window) {
	if ($.os.plus && $.os.android) {
		$.addBack({
			...
			handle: function() {
				...
				//popover
				if ($.targets._popover && $.targets._popover.classList.contains('iui-active')) {
					$($.targets._popover).popover('hide');
					return true;
				}
				//offcanvas
				var offCanvas = document.querySelector('.iui-off-canvas-wrap.iui-active');
				if (offCanvas) {
	...
	$.menu = function() {
		var menu = document.querySelector('.iui-action-menu');
		if (menu) {
							...
							$.plusReady(function() {
								...
								if (hasPulldown) {
									...
									if (parent) {
										...
										//父页面初始化pulldown
										parent.evalJS("iui&&iui(document.querySelector('.iui-content')).pullRefresh('" + JSON.stringify(downOptions) + "')");
									}
								}
							});
						} else {
...
/**
 * iui layout(offset[,position,width,height...])
 * @param {type} $
 * @param {type} window
 * @param {type} undefined
 * @returns {undefined}
...
(function($, document, undefined) {
	var CLASS_PULL_TOP_POCKET = 'iui-pull-top-pocket';
	var CLASS_PULL_BOTTOM_POCKET = 'iui-pull-bottom-pocket';
	var CLASS_PULL = 'iui-pull';
	var CLASS_PULL_LOADING = 'iui-pull-loading';
	var CLASS_PULL_CAPTION = 'iui-pull-caption';
	var CLASS_PULL_CAPTION_DOWN = 'iui-pull-caption-down';
	var CLASS_PULL_CAPTION_REFRESH = 'iui-pull-caption-refresh';
	var CLASS_PULL_CAPTION_NOMORE = 'iui-pull-caption-nomore';

	var CLASS_ICON = 'iui-icon';
	var CLASS_SPINNER = 'iui-spinner';
	var CLASS_ICON_PULLDOWN = 'iui-icon-pulldown';

	var CLASS_BLOCK = 'iui-block';
	var CLASS_HIDDEN = 'iui-hidden';
	var CLASS_VISIBILITY = 'iui-visibility';
...
(function($, window, document, undefined) {
	var CLASS_SCROLL = 'iui-scroll';
	var CLASS_SCROLLBAR = 'iui-scrollbar';
	var CLASS_INDICATOR = 'iui-scrollbar-indicator';
	var CLASS_SCROLLBAR_VERTICAL = CLASS_SCROLLBAR + '-vertical';
	var CLASS_SCROLLBAR_HORIZONTAL = CLASS_SCROLLBAR + '-horizontal';

	var CLASS_ACTIVE = 'iui-active';
	...
	var Scroll = $.Class.extend({
		...
		_initEvent: function(detach) {
			...
			var segmentedControl = this.wrapper.querySelector('.iui-segmented-control');
			if (segmentedControl) { //靠，这个bug排查了一下午，阻止hash跳转，一旦hash跳转会导致可拖拽选项卡的tab不见
				iui(segmentedControl)[detach ? 'off' : 'on']('click', 'a', $.preventDefault);
			}
	...
	$.fn.scroll = function(options) {
		...
		this.each(function() {
			...
			if (!id) {
				...
				if (self.classList.contains('iui-segmented-control')) {
					_options = $.extend(_options, {
						scrollY: false,
						scrollX: true,
						indicators: false,
						snap: '.iui-control-item'
					});
				}
...
(function($, window, document, undefined) {
	...
	var PullRefresh = $.Scroll.extend($.extend({
		...
		disablePullupToRefresh: function() {
			this._initPullupRefresh();
			this.bottomPocket.className = 'iui-pull-bottom-pocket' + ' ' + CLASS_HIDDEN;
 ...
 */
(function($, window) {
	var CLASS_SLIDER = 'iui-slider';
	var CLASS_SLIDER_GROUP = 'iui-slider-group';
	var CLASS_SLIDER_LOOP = 'iui-slider-loop';
	var CLASS_SLIDER_INDICATOR = 'iui-slider-indicator';
	var CLASS_ACTION_PREVIOUS = 'iui-action-previous';
	var CLASS_ACTION_NEXT = 'iui-action-next';
	var CLASS_SLIDER_ITEM = 'iui-slider-item';

	var CLASS_ACTIVE = 'iui-active';

	var SELECTOR_SLIDER_ITEM = '.' + CLASS_SLIDER_ITEM;
	var SELECTOR_SLIDER_INDICATOR = '.' + CLASS_SLIDER_INDICATOR;
	var SELECTOR_SLIDER_PROGRESS_BAR = '.iui-slider-progress-bar';
	...
	var Slider = $.Slider = $.Scroll.extend({
		...
		_handleSlide: function(e) {
			...
			if (!self.wrapper.classList.contains('iui-segmented-control')) {
				for (var i = 0, len = items.length; i < len; i++) {
					var item = items[i];
					if (item.parentNode === self.scroller) {
						if (i === _slideNumber) {
			...
			var indicatorWrap = self.wrapper.querySelector('.iui-slider-indicator');
			if (indicatorWrap) {
				if (indicatorWrap.getAttribute('data-scroll')) { //scroll
					$(indicatorWrap).scroll().gotoPage(detail.slideNumber);
				}
				var indicators = indicatorWrap.querySelectorAll('.iui-indicator');
				if (indicators.length > 0) { //图片轮播
					for (var i = 0, len = indicators.length; i < len; i++) {
						indicators[i].classList[i === detail.slideNumber ? 'add' : 'remove'](CLASS_ACTIVE);
					}
				} else {
					var number = indicatorWrap.querySelector('.iui-number span');
					if (number) { //图文表格
						number.innerText = (detail.slideNumber + 1);
					} else { //segmented controls
						var controlItems = indicatorWrap.querySelectorAll('.iui-control-item');
	...
	$.ready(function() {
		//		setTimeout(function() {
		$('.iui-slider').slider();
		$('.iui-scroll-wrapper.iui-slider-indicator.iui-segmented-control').scroll({
			scrollY: false,
			scrollX: true,
			indicators: false,
			snap: '.iui-control-item'
...
(function($, document) {
    ...
    $.plusReady(function() {
        ...
        var CLASS_PLUS_PULLREFRESH = 'iui-plus-pullrefresh';
        var CLASS_VISIBILITY = 'iui-visibility';
        var CLASS_HIDDEN = 'iui-hidden';
        var CLASS_BLOCK = 'iui-block';
        ...
        var PlusPullRefresh = $.Class.extend({
                        ...
                        webview.addEventListener("dragBounce", function(e) {
                            ...
                            switch (e.status) {
                                ...
                                case "dragEndAfterChangeOffset": //正在刷新状态
                                    ...
                                    webview.evalJS("iui&&iui.options.pullRefresh.down.callback()");
            ...
            endPulldownToRefresh: function() { //该方法是子页面调用的
                var webview = plus.webview.currentWebview();
                webview.parent().evalJS("iui&&iui(document.querySelector('.iui-content')).pullRefresh('" + JSON.stringify({
...
(function($, window, document, name) {
	var CLASS_OFF_CANVAS_LEFT = 'iui-off-canvas-left';
	var CLASS_OFF_CANVAS_RIGHT = 'iui-off-canvas-right';
	var CLASS_ACTION_BACKDROP = 'iui-off-canvas-backdrop';
	var CLASS_OFF_CANVAS_WRAP = 'iui-off-canvas-wrap';

	var CLASS_SLIDE_IN = 'iui-slide-in';
	var CLASS_ACTIVE = 'iui-active';


	var CLASS_TRANSITIONING = 'iui-transitioning';

	var SELECTOR_INNER_WRAP = '.iui-inner-wrap';
	...
	var OffCanvas = $.Class.extend({
		init: function(element, options) {
			...
			if (this.scroller) {
				...
				document.body.classList.add('iui-fullscreen'); //fullscreen
				this.refresh();
				this.initEvent();
			}
		},
		...
		refresh: function(offCanvas) {
			...
			this.scalable = this.classList.contains('iui-scalable') && !this.slideIn;
			this.scroller = this.wrapper.querySelector(SELECTOR_INNER_WRAP);
		...
		initEvent: function() {
			...
			if (this.classList.contains('iui-draggable')) {
	...
	$.ready(function() {
		$('.iui-off-canvas-wrap').offCanvas();
	});
})(iui, window, document, 'offcanvas');
/**
 * actions
 * @param {type} $
 * @param {type} name
...
(function($, name) {
	...
	var handle = function(event, target) {
		...
		if (className && ~className.indexOf(CLASS_ACTION)) {
			if (target.classList.contains('iui-action-back')) {
				event.preventDefault();
			}
			return target;
		}
...
(function($, window, document, name) {
	...
	window.addEventListener('tap', function(event) {
		if ($.targets.modal) {
			...
			$.targets.modal.classList.toggle('iui-active');
		}
	});
})(iui, window, document, 'modal');
/**
 * Popovers
 * @param {type} $
 * @param {type} window
...
(function($, window, document, name) {
	...
	var CLASS_POPOVER = 'iui-popover';
	var CLASS_POPOVER_ARROW = 'iui-popover-arrow';
	var CLASS_ACTION_POPOVER = 'iui-popover-action';
	var CLASS_BACKDROP = 'iui-backdrop';
	var CLASS_BAR_POPOVER = 'iui-bar-popover';
	var CLASS_BAR_BACKDROP = 'iui-bar-backdrop';
	var CLASS_ACTION_BACKDROP = 'iui-backdrop-action';
	var CLASS_ACTIVE = 'iui-active';
	var CLASS_BOTTOM = 'iui-bottom';
	...
	var togglePopover = function(popover, anchor, state) {
		...
		var _popover = document.querySelector('.iui-popover.iui-active');
		if (_popover) {
...
(function($, window, document, name, undefined) {
	...
	var CLASS_CONTROL_ITEM = 'iui-control-item';
	var CLASS_SEGMENTED_CONTROL = 'iui-segmented-control';
	var CLASS_SEGMENTED_CONTROL_VERTICAL = 'iui-segmented-control-vertical';
	var CLASS_CONTROL_CONTENT = 'iui-control-content';
	var CLASS_TAB_BAR = 'iui-bar-tab';
	var CLASS_TAB_ITEM = 'iui-tab-item';
	var CLASS_SLIDER_ITEM = 'iui-slider-item';
...
(function($, window, name) {
	...
	var CLASS_SWITCH = 'iui-switch';
	var CLASS_SWITCH_HANDLE = 'iui-switch-handle';
	var CLASS_ACTIVE = 'iui-active';
	var CLASS_DRAGGING = 'iui-dragging';

	var CLASS_DISABLED = 'iui-disabled';
...
(function($, window, document) {
	...
	var CLASS_ACTIVE = 'iui-active';
	var CLASS_SELECTED = 'iui-selected';
	var CLASS_GRID_VIEW = 'iui-grid-view';
	var CLASS_RADIO_VIEW = 'iui-table-view-radio';
	var CLASS_TABLE_VIEW_CELL = 'iui-table-view-cell';
	var CLASS_COLLAPSE_CONTENT = 'iui-collapse-content';
	var CLASS_DISABLED = 'iui-disabled';
	var CLASS_TOGGLE = 'iui-switch';
	var CLASS_BTN = 'iui-btn';

	var CLASS_SLIDER_HANDLE = 'iui-slider-handle';
	var CLASS_SLIDER_LEFT = 'iui-slider-left';
	var CLASS_SLIDER_RIGHT = 'iui-slider-right';
	var CLASS_TRANSITIONING = 'iui-transitioning';
	...
	var radioOrCheckboxClick = function(event) {
		...
		if (classList.contains('iui-radio')) {
		...
		} else if (classList.contains('iui-checkbox')) {
	...
	//fixed hashchange(android)
	window.addEventListener($.EVENT_CLICK, function(e) {
		if (cell && cell.classList.contains('iui-collapse')) {
			e.preventDefault();
		}
	});
	...
	window.addEventListener('tap', function(event) {
		...
		if (classList.contains('iui-collapse') && !cell.parentNode.classList.contains('iui-unfold')) {
			if (!preventDefaultException.test(event.target.tagName)) {
				event.detail.gesture.preventDefault();
			}

			if (!classList.contains(CLASS_ACTIVE)) { //展开时,需要收缩其他同类
				var collapse = cell.parentNode.querySelector('.iui-collapse.iui-active');
				if (collapse) {
 ...
 */
(function($, window, document) {
    var CLASS_POPUP = 'iui-popup';
    var CLASS_POPUP_BACKDROP = 'iui-popup-backdrop';
    var CLASS_POPUP_IN = 'iui-popup-in';
    var CLASS_POPUP_OUT = 'iui-popup-out';
    var CLASS_POPUP_INNER = 'iui-popup-inner';
    var CLASS_POPUP_TITLE = 'iui-popup-title';
    var CLASS_POPUP_TEXT = 'iui-popup-text';
    var CLASS_POPUP_INPUT = 'iui-popup-input';
    var CLASS_POPUP_BUTTONS = 'iui-popup-buttons';
    var CLASS_POPUP_BUTTON = 'iui-popup-button';
    var CLASS_POPUP_BUTTON_BOLD = 'iui-popup-button-bold';
    var CLASS_POPUP_BACKDROP = 'iui-popup-backdrop';
    var CLASS_ACTIVE = 'iui-active';
...
(function($, document) {
	var CLASS_PROGRESSBAR = 'iui-progressbar';
	var CLASS_PROGRESSBAR_IN = 'iui-progressbar-in';
	var CLASS_PROGRESSBAR_OUT = 'iui-progressbar-out';
	var CLASS_PROGRESSBAR_INFINITE = 'iui-progressbar-infinite';

	var SELECTOR_PROGRESSBAR = '.iui-progressbar';
	...
	/**
	 * 创建并显示进度条 
	 * @param {Object} container  可选，默认body，支持selector,DOM Node,iui wrapper
	...
	$.fn.progressbar = function(options) {
		...
		this.each(function() {
			var self = this;
			var progressbarApi = self.iui_plugin_progressbar;
			if (!progressbarApi) {
				self.iui_plugin_progressbar = progressbarApi = {
					options: options,
...
(function($, window, document) {
	var CLASS_ICON = 'iui-icon';
	var CLASS_ICON_CLEAR = 'iui-icon-clear';
	var CLASS_ICON_SPEECH = 'iui-icon-speech';
	var CLASS_ICON_SEARCH = 'iui-icon-search';
	var CLASS_ICON_PASSWORD = 'iui-icon-eye';
	var CLASS_INPUT_ROW = 'iui-input-row';
	var CLASS_PLACEHOLDER = 'iui-placeholder';
	var CLASS_TOOLTIP = 'iui-tooltip';
	var CLASS_HIDDEN = 'iui-hidden';
	var CLASS_FOCUSIN = 'iui-focusin';
		...
		if (this.sliderActionClass) {
			...
			if (this.searchActionClass) {
				element.addEventListener('focus', function() {
					element.parentNode.classList.add('iui-active');
				});
				element.addEventListener('blur', function() {
					if (!element.value.trim()) {
						element.parentNode.classList.remove('iui-active');
					}
				});
			}
		}
	...
	Input.prototype.passwordActionClick = function(event) {
		...
		this.passwordAction.classList.toggle('iui-active');
		event.preventDefault();
	};
	...
	$.fn.input = function(options) {
		...
		this.each(function() {
			...
			if (this.type === 'range' && row.classList.contains('iui-input-range')) {
				actions.push('slider');
			} else {
				var classList = this.classList;
				if (classList.contains('iui-input-clear')) {
					actions.push('clear');
				}
				if (!($.os.android && $.os.stream) && classList.contains('iui-input-speech')) {
					actions.push('speech');
				}
				if (classList.contains('iui-input-password')) {
					actions.push('password');
				}
				if (this.type === 'search' && row.classList.contains('iui-search')) {
					actions.push('search');
				}
			}
...
(function($, window) {
	...
	var Transparent = function(element, options) {
		...
		var color = getColor(iui.getStyles(this.element, 'backgroundColor'));
		if (color.length) {
			this._R = color[0];
			this._G = color[1];
			this._B = color[2];
	...
	Transparent.prototype.destory = function() {
		...
		this.element.iui_plugin_transparent = null;
	};
	$.fn.transparent = function(options) {
		options = options || {};
		var transparentApis = [];
		this.each(function() {
			var transparentApi = this.iui_plugin_transparent;
			if (!transparentApi) {
				...
				transparentApi = this.iui_plugin_transparent = new Transparent(this, options);
			}
	...
	$.ready(function() {
		$('.iui-bar-transparent').transparent();
	});
})(iui, window);
/**
 * 数字输入框
...
(function($) {
    ...
    var holderClassName = 'iui-numbox';
    var plusClassSelector = '.iui-btn-numbox-plus,.iui-numbox-btn-plus';
    var minusClassSelector = '.iui-btn-numbox-minus,.iui-numbox-btn-minus';
    var inputClassSelector = '.iui-input-numbox,.iui-numbox-input';
...
(function($, window, document) {
    ...
    var defaultOptions = {
        ...
        loadingIcon: 'iui-spinner' + ' ' + 'iui-spinner-white', //图标，可为空
    ...
    var Button = function(element, options) {
        ...
        if (this.options.loadingIcon === null) {
            this.options.loadingIcon = 'iui-spinner';
            if ($.getStyles(this.element, 'color') === 'rgb(255, 255, 255)') {
                this.options.loadingIcon += ' ' + 'iui-spinner-white';
            }
        }
    ...
    $.fn.button = function(state) {
        var buttonApis = [];
        this.each(function() {
            var buttonApi = this.iui_plugin_button;
            if (!buttonApi) {
                ...
                this.iui_plugin_button = buttonApi = new Button(this, {
